...
contract TestHelper is Test {
    ...
    function setUp() public virtual {
        ...
        factory2 = VaultFactory(address(new TransparentUpgradeableProxy(implementation, address(1), "")));
        factory2.initialize(owner);
        ...
        factory2.setVaultImplementation(IVaultFactory.VaultType.Simple, address(new SimpleVault(factory2)));
        factory2.setVaultImplementation(IVaultFactory.VaultType.Oracle, address(new OracleVault(factory2)));

        // factory2.setStrategyImplementation(IVaultFactory.StrategyType.Default, address(new StepStrategy(factory2)));
        vm.stopPrank();
    ...
    function depositToVault(address newVault, address from, uint256 amountX, uint256 amountY) public {
        ...
        vm.prank(from);
        IBaseVault(newVault).deposit(amountX, amountY, 0);
    }
    ...
    function depositNativeToVault(address newVault, address from, uint256 amountX, uint256 amountY) public {
        ...
        if (isX) {
            ...
            vm.prank(from);
            IBaseVault(newVault).depositNative{value: amountX}(amountX, amountY, 0);
        } else {
            vm.prank(from);
            tokenX.approve(newVault, amountX);

            vm.prank(from);
            IBaseVault(newVault).depositNative{value: amountY}(amountX, amountY, 0);
        }
    }
    ...
    function linkVaultToStepStrategy(address newVault, address newStrategy) public {
        vm.prank(owner);
        factory2.linkVaultToStrategy(IBaseVault(newVault), newStrategy);
    }
}
